...
with open(FILENAME,'r') as f:
    for line in f:
        ...
        # unused: next chain_len values of data are raw throughputs for each chain

        per_chain_len = {}
        if packet_sz in per_packet_sz:
            per_chain_len = per_packet_sz[packet_sz]
        
        per_chain_len[chain_len * chain_len] = throughput
        per_packet_sz[packet_sz] = per_chain_len

filename = 'outputs/figure_8_plots.png'
...
plt.title('Recreation of Figure 8 for ' + str(sim_time) + 's')
plt.ylabel('Chain Throughput (Mbps)')
plt.xlabel('Lattice Size (Number of Nodes)')
plt.grid()
plt.legend()
